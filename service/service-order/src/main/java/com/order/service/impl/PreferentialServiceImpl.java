...
import com.order.dao.PreferentialMapper;
import com.order.pojo.Preferential;
import com.order.service.PreferentialService;
...
public class PreferentialServiceImpl implements PreferentialService {
    ...
    public Example createExample(Preferential preferential){
        ...
        if(preferential!=null){
            ...
            // 消费金额
            if(!StringUtils.isEmpty(preferential.getBuyBuyMoney())){
                    criteria.andEqualTo("buyBuyMoney",preferential.getBuyBuyMoney());
            }
            // 优惠金额
            if(!StringUtils.isEmpty(preferential.getPrePreMoney())){
                    criteria.andEqualTo("prePreMoney",preferential.getPrePreMoney());
            }
            // 品类ID
            if(!StringUtils.isEmpty(preferential.getCategoryCategoryId())){
                    criteria.andEqualTo("categoryCategoryId",preferential.getCategoryCategoryId());
            }
            // 活动开始日期
            if(!StringUtils.isEmpty(preferential.getStartStartTime())){
                    criteria.andEqualTo("startStartTime",preferential.getStartStartTime());
            }
            // 活动截至日期
            if(!StringUtils.isEmpty(preferential.getEndEndTime())){
                    criteria.andEqualTo("endEndTime",preferential.getEndEndTime());
            }
            // 状态
